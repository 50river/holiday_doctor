...
function jsonLoaded(response){
    ...
    for (var i = 0; i < len; i++) {
        ...
        if (cat_check != collection.category) {
            j++;
            var insertElement = $('<div class="panel panel-default">' +
                              ...
                              '<a data-toggle="collapse" data-parent="#accordion" href="#'+collection.category.charCodeAt(0)+ '">' +
                              collection.category + '</a></h4></div>' +
                              '<div id="' + collection.category.charCodeAt(0) + '" class="panel-collapse collapse"><div class="panel-body"><div class="' + j + '"></div></div></div></div>')

            $("#accordion").append(insertElement);
            cat_check = collection.category;
        };
